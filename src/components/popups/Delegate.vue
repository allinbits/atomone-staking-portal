...
const delegationAmount = ref<number | null>(null);
...
const resetDeposit = () => (delegationAmount.value = null);
...
const { createDelegation } = useValidators();
const { used } = useWallet();

const signDelegation = async (isCLI = false) => {
  if (!delegationAmount.value || delegationAmount.value <= 0) return;
  ...
  try {
    ...
    const depot = await createDelegation(delegationOptions, isCLI);
      ...
      <div class="bg-grey-400 w-full rounded-md max-h-screen overflow-auto">
        <div class="px-10 py-12 bg-grey-400 rounded w-screen max-w-[25rem]">
          <div v-show="displayState === 'pending'" class="flex flex-col gap-6 relative">
            <span class="text-gradient font-termina text-700 text-center">{{ $t("components.Delegate.cta") }}</span>
            <div class="flex flex-col gap-10">
              <div>
                <div class="flex flex-col gap-10">
                  <form class="flex flex-col items-center gap-2">
                    <UiInput v-model="delegationAmount" type="number" placeholder="e.g. 50" label="Amount to stake"
                      :min="0" :max="Infinity" class="w-full justify-end" />
                  </form>
                </div>
              </div>

              <div v-if="!transacting" class="flex flex-col gap-4">
                <div v-show="(delegationAmount ?? -1) > 0" class="flex flex-col gap-4">
                  <button class="px-6 py-4 rounded link-gradient text-dark text-300 text-center w-full"
                    @click="signDelegation(true)">
                    {{ $t("ui.actions.cli") }}
                  </button>
          ...
          <div v-show="displayState === 'delegated'">
            <UiInfo :title="$t('components.Delegate.delegated')">
              <div class="text-500 text-center font-semibold mb-8 w-full">
                {{ delegationAmount }} {{ delegationDenomDisplay }}
              </div>
            </UiInfo>
